...
JIA:
    ...
    If thisErrno <> 0 Then
        ...
        tmp = vb_igeterrstr(thisErrno, myerrstr)
        Err.Description = myerrstr
        'Err.Raise (thisErrno) 'Raise the error    '*******jia
JIA2:
        MsgBox "RESTART MMC-2, AND CLICK OK.", vbCritical
        Finalized_Instrument MMC_2XY_ID
        Finalized_Instrument MMC_2Z_ID
        If Not Initialize_Instrument(MMC_2XY_ID, MMC_2_XY_AD) Then GoTo JIA2
        If Not Initialize_Instrument(MMC_2Z_ID, MMC_2_Z_AD) Then GoTo JIA2
        Call vb_icauseerr(id1, 0, 0)
        thisErrno = vb_igeterrno()
        GoTo JIA
    End If
